...
MODEL_SFT = os.getenv("OPENROUTER_MODEL_SAFETY","meta-llama/llama-3.1-70b-instruct")
if not OR_KEY: raise SystemExit("Missing OPENROUTER_API_KEY")
...
def process_text(payload: Optional[str]) -> None:
    ...
    try:
        ...
        print("[ERROR] expected JSON with user_text and assistant_text"); return

    user_text = p.get("user_text","")
    assistant_text = p.get("assistant_text","")
    emerg_gate = bool(p.get("emergency_gate_hit", False))
